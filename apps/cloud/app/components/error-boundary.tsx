    ...
    <main className="container mx-auto space-y-2 p-4 sm:p-8">
      ...
      {stack && (
        <div className="rounded-lg bg-red-600/10 p-4 text-red-600">
          <pre className="max-h-96 w-full overflow-x-auto overflow-y-auto text-sm">{stack}</pre>
        </div>
      )}
...
export function GeneralErrorBoundary({ className }: { className?: string }) {
  ...
  if (isDev && stack && error && error instanceof Error) {
    return <DevErrorPage title={title} description={description} stack={stack} />;
  }

  return <ErrorPage title={title} description={description} className={className} />;
}
