...
const SidebarMenuLink = ({ item, href }: { item: NavItemLink; href: string }) => {
  ...
  return (
    <SidebarMenuItem>
      <SidebarMenuButton asChild isActive={isActiveLink(href, item.url)} tooltip={item.title}>
          ...
          <SidebarMenuSub>
            {item.items.map((subItem) => (
              <SidebarMenuSubItem key={subItem.title}>
                <SidebarMenuSubButton asChild isActive={isActiveLink(href, subItem.url)}>
        ...
        <DropdownMenuContent side="right" align="start" sideOffset={4}>
          ...
          {item.items.map((sub) => (
            <DropdownMenuItem key={`${sub.title}-${sub.url}`} asChild>
              <Link to={sub.url} className={isActiveLink(href, sub.url) ? "bg-secondary" : ""}>
                {sub.icon && <sub.icon />}
    ...
    <SidebarGroup>
      ...
      <SidebarMenu>
        {items.map((item: NavItemUnion) => {
          ...
          if (isNavLink(item)) return <SidebarMenuLink key={key} item={item} href={href} />;
