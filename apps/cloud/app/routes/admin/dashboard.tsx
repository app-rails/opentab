...
export async function loader(_: Route.LoaderArgs) {
  ...
  const totalContentPromise = new Promise<number>((resolve) => setTimeout(() => resolve(100), 30));
  const categoriesCountPromise = new Promise<number>((resolve) =>
    setTimeout(() => resolve(392), 60),
  );
  const tagsCountPromise = new Promise<number>((resolve) => setTimeout(() => resolve(678), 90));
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4 *:data-[slot=card]:shadow-xs sm:grid-cols-2 xl:grid-cols-4">
        <Suspense fallback={<CardSkeleton />}>
          <Await resolve={usersCountPromise} errorElement={<div>Failed to load users count.</div>}>
            {(usersCount) => (
        ...
        <Suspense fallback={<CardSkeleton />}>
          <Await resolve={tagsCountPromise} errorElement={<div>Failed to load tags count.</div>}>
            {(tagsCount) => (
              <Card className="@container/card dark:bg-accent/30">
                <CardHeader>
